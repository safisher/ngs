...
ngsCmd_HTSEQ() { 
	...
	prnCmd "python -m HTSeq.scripts.count --format=bam --order=name --mode=union --stranded=$ngsLocal_HTSEQ_STRANDED --type=intron --idattr=$ngsLocal_HTSEQ_ID_ATTR $SAMPLE/$ngsLocal_HTSEQ_INP_DIR/$ngsLocal_HTSEQ_INP_FILE $HTSEQ_REPO/$SPECIES.gz > $SAMPLE/htseq/$SAMPLE.htseq.introns.out 2>&1 &"
	if ! $DEBUG; then 
	    python -m HTSeq.scripts.count --format=bam --order=name --mode=union --stranded=$ngsLocal_HTSEQ_STRANDED --type=intron --idattr=$ngsLocal_HTSEQ_ID_ATTR $SAMPLE/$ngsLocal_HTSEQ_INP_DIR/$ngsLocal_HTSEQ_INP_FILE $HTSEQ_REPO/$SPECIES.gz > $SAMPLE/htseq/$SAMPLE.htseq.introns.out 2>&1 &
	fi
